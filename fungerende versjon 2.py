# -*- coding: utf-8 -*-
"""
Created on Tue Oct  7 12:14:41 2025
...
# ---------- OPPGAVE 6 og 7 ----------
def lagre_til_fil(semestre, register):
    """Lagrer emner og studieplan til en enkel JSON-fil."""
    try:
        with open("studieplan.json", "w", encoding="utf-8") as fil:
            json.dump({"semestre": semestre, "register": register}, fil, ensure_ascii=False, indent=2)
        print("Studieplan lagret til 'studieplan.json'.")
    except Exception as e:
        print(f"Feil ved lagring: {e}")

def les_fra_fil():
    """Leser emner og studieplan fra fil."""
    try:
        with open("studieplan.json", "r", encoding="utf-8") as fil:
            data = json.load(fil)
        print("Studieplan lest fra 'studieplan.json'.")
        return data["semestre"], data["register"]
    except FileNotFoundError:
        print("Fant ikke filen 'studieplan.json'. Lagre først med valg 6.")
    except Exception as e:
...
def main():
    ...
    global semestre, register  # slik at de kan oppdateres etter lesing
    ...
    while True:
        ...
        elif valg == "6":  # OPPGAVE 6
            lagre_til_fil(semestre, register); pause()

        elif valg == "7":  # OPPGAVE 7
            nye_semestre, nytt_register = les_fra_fil()
            if nye_semestre is not None and nytt_register is not None:
                semestre = nye_semestre
                register = nytt_register
            pause()

        elif valg == "8":  # OPPGAVE 8
